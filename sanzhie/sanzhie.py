...
def find_close_button(driver):
    ...
    time.sleep(5)
    print(f"[DEBUG] 屏幕高度: {screen_height}")

    search_criteria = [
        # (By.ID, "com.guokun.darenzhushou:id/ksad_auto_close_btn"),
        # (By.ID, "com.guokun.darenzhushou:id/anythink_myoffer_btn_close_id"),
        (By.CLASS_NAME, "android.widget.ImageView"),
        (By.XPATH, "//android.widget.TextView[contains(@text, '跳过')]")
    ]

    found_elements = []
    max_attempts = 20
    attempts = 0
    skip_found = False  # 用于记录是否检测到“跳过”按钮
    ...
    while attempts < max_attempts:
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            for by, value in search_criteria:
                print(f"[DEBUG] 查找条件: {by}, {value}")
                futures.append(executor.submit(
                    lambda b=by, v=value: WebDriverWait(driver, 5).until(
                        EC.presence_of_all_elements_located((b, v))
                    )
                ))
            ...
            for future in as_completed(futures):
                try:
                    ...
                    for element in elements:
                        try:
                            # time.sleep(1)  # 稍作等待，防止元素变动
                            size = element.size
                            location = element.location
                            class_name = element.get_attribute("class")
                            print(f"[DEBUG] 检查元素: [元素名: {class_name}, 大小: {size}, 坐标: {location}]")

                            # 如果是“跳过”按钮，设置标志位并重新查找
                            if class_name == "android.widget.TextView" and "跳过" in element.text:
                                if skip_count < 1:
                                    ...
                                    # print("[DEBUG] 检测到'跳过'按钮，再次跳过...")
                                    skip_count += 1
                                    skip_found = True
                                    break  # 跳过处理，继续寻找其他元素

                            if (15 < size['width'] < 60 and 15 < size['height'] < 60 and location['y'] < screen_height / 2):
            ...
            if skip_found:
                found_elements.clear()  # 清除当前找到的元素，重新查找
                time.sleep(10)
                attempts += 1
                continue  # 重新开始查找关闭按钮元素
