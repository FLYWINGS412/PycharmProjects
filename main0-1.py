...
# 取消关注任务
def perform_unfollow_task(accounts, follow_list):
    for account in accounts:
        print(f"账号 {account['phone']} 开始取消关注任务")
        ...
        if not execute_task(driver, wait, width, height, tasks.unfollow, account, follow_list):
            print(f"账号 {account['phone']} 执行取消关注任务失败，程序继续。")

# 点赞任务
def perform_like_task(accounts, follow_list):
    for account in accounts:
        print(f"账号 {account['phone']} 开始点赞任务")
        ...
        if not execute_task(driver, wait, width, height, tasks.like, account, follow_list):
            print(f"账号 {account['phone']} 执行点赞任务失败，程序继续。")
...
def main():
    ...
    if task_type == '1':
        ...
        perform_like_task(accounts, follow_list)

    elif task_type == '5':
        perform_unfollow_task(accounts, follow_list)
