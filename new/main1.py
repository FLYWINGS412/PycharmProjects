...
def main():
    ...
    tasks_list = [tasks.handle_home_page_video, tasks.handle_home_page_video, tasks.handle_home_page_video]
    ...
    if task_type == '1':
        ...
        if user_input.isdigit():
            ...
            _, start_account_index = auth.get_progress()
        perform_tasks(accounts, [tasks_list[start_task_index]], start_task_index, start_account_index, is_single_task=True)

    elif task_type == '2':
        print("请选择任务:")
        print("1. 从第一个任务开始")
        print("2. 从上次的任务开始")
        task_choice = input("请输入选择序号: ")
        if task_choice == '1':
            start_task_index = 0
