...
def handle_display_page(driver, wait, width, height):
    try:
        ...
        while time.time() - start_time < timeout:
            # 检查弹窗
            if not handle_popups(driver, popup_texts):
                continue
            ...
            # 检查倒计时是否消失
            if not check_countdown_disappeared(driver, height):
                print("倒计时全部结束")
                break
...
def check_countdown_disappeared(driver, height):
    try:
                ...
                try:
                    ...
                    if top_y < height * 0.15:
                        ...
                        WebDriverWait(driver, 3).until(
                            lambda driver: any(x in element_to_wait.text for x in ['0 s', '0s']) or not element_to_wait.is_displayed()
                        )
                        print("倒计时结束。")
                        return True
                except StaleElementReferenceException:
        ...
        elif driver.find_elements(MobileBy.XPATH, "//android.widget.TextView[string-length(@text) = 1 or string-length(@text) = 2]"):
            text_views = driver.find_elements(MobileBy.XPATH, "//android.widget.TextView[string-length(@text) = 1 or string-length(@text) = 2]")
            for text_view in text_views:
                try:
                    location = text_view.location
                    size = text_view.size
                    ...
                    if top_y < height * 0.15:
                        ...
                        print(f"等待倒计时元素消失前的状态: 文本='{element_to_wait.text}'")
                        WebDriverWait(driver, 3).until(
                            lambda driver: '0' in element_to_wait.text or not element_to_wait.is_displayed()
                        )
                        print("倒计时结束。")
                        return True
                except StaleElementReferenceException:
        ...
        # 检查其他可能的倒计时元素
        if driver.find_elements(MobileBy.ID, "com.xiangshi.bjxsgc:id/anythink_myoffer_count_down_view_id"):
            element_to_wait = (MobileBy.ID, "com.xiangshi.bjxsgc:id/anythink_myoffer_count_down_view_id")
            WebDriverWait(driver, 3).until(EC.invisibility_of_element_located(element_to_wait))
            print("特定ID的倒计时元素已消失。")
            return True
...
def handle_popups(driver, popup_texts):
