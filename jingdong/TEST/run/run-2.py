...
# 封装 main.py 的核心逻辑为函数
def run_main_task(desired_caps, update_log_callback, stop_event):
    ...
    try:
        ...
        # 启动执行任务的线程
        task_thread = Thread(target=perform_tasks, args=(driver,), daemon=True)
        task_thread.start()

        while task_thread.is_alive():
            if stop_event.is_set():
                print("收到停止信号，关闭驱动...")
                driver.quit()  # 关闭驱动会导致 perform_tasks 异常退出
                break

            time.sleep(0.1)
            log_output = mystdout.getvalue()
            if log_output:
                update_log_callback(log_output)
                mystdout.truncate(0)
                mystdout.seek(0)
...
for device_name in config.keys():
    ...
    stop_events[device_name] = Event()  # 为每个设备创建停止事件
...
def start_device(device_name, button):
    ...
    # 创建一个新的停止事件
    stop_events[device_name].clear()

    # 创建一个新线程来运行自动化任务
    thread = Thread(
        target=lambda: run_main_task(
            desired_caps,
            lambda log: update_log(device_name, log),
            stop_events[device_name]
        ),
        daemon=True
    )
    thread.start()
...
def stop_device(device_name, button):
    if device_name in device_threads and device_threads[device_name].is_alive():
        ...
        stop_events[device_name].set()  # 设置停止事件
        device_threads[device_name].join(timeout=5)  # 等待线程结束
